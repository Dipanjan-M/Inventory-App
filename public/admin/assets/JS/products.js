function edit_product(p_id, p_name, p_cat, p_unit_price, p_stock) {
    ...
    $.get("fetch_all_products.php", function(data, status) {
        ...
        products.forEach((product) => {
            ...
            elem.innerHTML += `<tr class="` + legend + `" align="center">
											...
											<td><span class="text-primary" style="cursor: pointer;" onclick="edit_product('` + product['id'] + `','` + product['p_name'] + `','` + product['category'] + `','` + product['unit_price'] + `','` + product['total_stock'] + `');">Edit</span> | <span class="text-danger" style="cursor: pointer;" onclick="delete_product('` + product['id'] + `');">Delete</span></td>
									   </tr>`;
        });
    });
}
